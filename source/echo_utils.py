...
def load_data(include_times=False, histogram_equalisation=False):
    ...
    data_info['diagnosis'] = data_info['id'].apply(lambda s: s[-1]).astype('string')
    ...
    for view in ['2CH', '4CH']:
        ...
        for index, row in data_info.iterrows():
            ...
            if histogram_equalisation:
                cache_folder = './cache/AMI_TTS_histeq/' + diagnosis + '/' + str(number)
            else:
                cache_folder = './cache/AMI_TTS/' + diagnosis + '/' + str(number)

            # load data from cache or directly from video files
            if os.path.exists(cache_folder + '/' + view):
                # load from cache
                times = np.loadtxt(cache_folder + '/' + view + '/times', delimiter='\n')

                frames = []
                for i, _ in enumerate(times):
                    img = cv.imread(cache_folder + '/' + view + '/' + str(i) + '.bmp')
                    frames.append(img[:, :, 0])
                ...
                # to cache
                os.makedirs(cache_folder + '/' + view + '/', exist_ok=True)
                for i, img in enumerate(frames):
                    cv.imwrite(cache_folder + '/' + view + '/' + str(i) + '.bmp', img)
                np.savetxt(cache_folder + '/' + view + '/times', times, delimiter='\n')

            frames_by_view[view][index] = frames
            times_by_view[view][index] = times

    if include_times is True:
        return (data_info,
                frames_by_view['2CH'], frames_by_view['4CH'],
...
def load_data_ami_tts_112(histogram_equalisation=False, view='4CH'):
    ...
    for diagnosis in ['AMI', 'TTS']:
        os.makedirs(video_cache_folder + '/' + diagnosis, exist_ok=True)
    ...
    for index, row in data_info.iterrows():
        number = row['id'][:-1]
        diagnosis = 'AMI' if row['diagnosis'] == 'A' else 'TTS'
        filepath = './data/AMI_TTS_112/' + view + '/' + diagnosis + '/' + str(number) + '.npy'
        filepath_original = './data/AMI_TTS/' + diagnosis + '/' + str(number) + '/' + view + '.avi'
        filepath_cached = video_cache_folder + '/' + diagnosis + '/' + str(number) + '.npz'
...
def load_data_ami_tts_v2(cohort, histogram_equalisation=False,
    ...
    video_cache_folder = './cache/AMI_TTS_' + str(shape[0]) + '_' + str(shape[1]) + '_v2'
    if histogram_equalisation:
    ...
    video_info = pd.read_csv('./data/AMI_TTS_v2/video_info.csv',
                             index_col=['cohort', 'name', 'disease', 'chamber'])
    patient_data = pd.read_csv('./data/AMI_TTS_v2/patient_data.csv', index_col='globalId')
    patient_data['diagnosis'] = patient_data['TTS'].map(lambda b: 'T' if b == 1 else 'A')
    ...
    for index, row in video_info.iterrows():
        ...
        filepath_original = './data/AMI_TTS_v2/cohort' + str(cohort) + '/' + view + '/' + disease_i + '/' + str(original_name) + '.npy.npz'
        filepath_cached = video_cache_folder + '/' + disease_i + '/' + str(name_i) + '.npz'
...
def load_data_ami_tts_v3(cohort, view='4CH', filepaths_only=True):
    ...
    for index, row in video_info.iterrows():
        ...
        global_id = str(cohort) + '_' + str(name_i) + '_' + disease_i
        filepath_original = './data/AMI_TTS_v3/cohort' + str(cohort) + '/manual_crop/' + disease_i + '/' + str(original_name) + '/' + chamber_i + '.avi'
        filepath_cached = video_cache_folder + '/' + disease_i + '/' + str(name_i) + '.npz'
        ...
        if not os.path.exists(filepath_cached):
            ...
            # cache data
            np.savez(filepath_cached, frames=frames, frame_duration=frame_duration)
...
def load_data_echonet(path="./data/EchoNet-Dynamic/",
    ...
    def _basename_wo_ext(s):
        s = str(s)
        return os.path.splitext(s)[0]

    data_info['globalID'] = data_info['FileName'].apply(_basename_wo_ext).astype('str')
    ...
    for index, row in data_info.iterrows():
        ...
        filepath_cached = os.path.join(video_cache_folder, index + '.npz')
...
def show_clip(frames, rate=10, width=300, height=300):
    ...
    filepath = './tmp/' + id + '.mp4'
    skvideo.io.vwrite(
        filepath, frames, inputdict={'-r': str(rate)},
        outputdict={'-vcodec': 'libx264', '-pix_fmt': 'yuv420p', '-r': str(rate)}
    )
...
def save_video(frames, file_path, fps=30, display_video=False):
    ...
    try:
        ...
        skvideo.io.vwrite(
            file_path, frames, inputdict={'-r': str(fps)},
            outputdict={'-vcodec': 'libx264', '-pix_fmt': 'yuv420p', '-r': str(fps)},
            backend='ffmpeg', verbosity=1
        )
...
def get_val_dataset(files, val_batch_size=128):
    ...
    def _val_dataset_generator():
        for file in files:
            ...
            frames = np.array([cv.resize(x, (112, 112)) for x in frames]).astype('float32')
            # standardize data
            frames -= np.mean(frames)
            frames /= np.std(frames)
            for f in frames:
